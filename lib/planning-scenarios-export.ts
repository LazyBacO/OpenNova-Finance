...
export const exportPlanningScenariosCsv = ({
  age,
  income,
  savings,
  returnRate,
...
export const exportPlanningScenariosPng = ({
  results,
  baselineId,
  projectionYears,
  formatCurrency,
  ...
  results.forEach((scenario, index) => {
    ...
    context.globalAlpha = scenario.id === baselineId ? 1 : 0.7
  ...
  const baselineValue = results.find((item) => item.id === baselineId)?.finalValue ?? 0
  ...
  results.forEach((scenario, rowIndex) => {
    ...
    const diff = scenario.id === baselineId ? 0 : scenario.finalValue - baselineValue
    context.fillStyle = foregroundColor
    context.fillText(
